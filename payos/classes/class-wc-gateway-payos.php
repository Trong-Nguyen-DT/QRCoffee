...
class WC_Gateway_payOS extends WC_Payment_Gateway
{
	...
	static private $URL_CONFIRM_WEBHOOK = "https://api-app.payos.vn/confirm-webhook";
	...
	public function __construct()
	{
		...
		add_action('woocommerce_thankyou_payos', array($this, 'thankyou_page'));
		// Customer Emails.
		...
		if (!isset($payos_gateway_settings['client_id']) || strlen($payos_gateway_settings['client_id']) == 0
			...
			|| !isset($payos_gateway_settings['checksum_key']) || strlen($payos_gateway_settings['checksum_key']) == 0)
			{
				echo '<div class="notice notice-warning">
					<p>' . __('payOS has not been set up yet!', 'payos') . '</p>
				</div>';
			}
	}
		...
		if (is_array($_REQUEST['payos_gateway_settings'])) {
			...
			if ($create_webhook) {
				...
				$this->message = '<div class="updated notice"><p><strong>' .
									__('Successful webhook registration', 'payos') .
									'</p></strong></div>';
				...
				__('Webhook creation failed', 'payos') .
				'</p></strong></div>';
			}
			// Message for use
			$this->message .=
			'<div class="updated notice"><p><strong>' .
			__('Settings saved', 'payos') .
			'</p></strong></div>';
		} else {
			$this->update_payos_gateway_settings("no", self::$default_settings['gateway_info']);
			$this->message =
			'<div class="error notice"><p><strong>' .
			__('Can not save settings! Please refresh this page.', 'payos') .
			'</p></strong></div>';
		}
	}
...
?>
		...
		'
		<tr>
			<th scope="row">' . __('Connection Information', 'payos') . '</th>
			<td class="forminp" id="payos_gateway_settings">
				...
				if ($this->payos_gateway_settings['link_webhook'] == 'no') {
					$payment_gateway_config .= __('No Connection', 'payos') . '</button>';
				}
				
				else {	
					$payment_gateway_config .=  __('Đã kết nối', 'payos') . '</button>
					<div id="payos_gateway_info" style="padding: 20px; background-color: #bdbdbd; color: black; border: 1px solid; max-width: 500px; border-radius: 10px; display: none">
						...
						$payment_gateway_config .= '>
							<li><b>' . __('Gateway name', 'payos') . ': </b>' . esc_html($this->payos_gateway_settings['gateway_info']['name']) . '</li>
							<li><b>' . __('Account number', 'payos') . ': </b>' . esc_html($this->payos_gateway_settings['gateway_info']['account_number']) . '</li>
							<li><b>' . __('Account name', 'payos') . ': </b>' . esc_html($this->payos_gateway_settings['gateway_info']['account_name']) . '</li>	
							<li><b>' . __('Bank', 'payos') . ': </b>' . esc_html($this->payos_gateway_settings['gateway_info']['bank_name']) . '</li>	
						</ul>
					</div>';
				} 
			...
			$payment_gateway_config .= '
				...
				<button id="toggle_payos_gateway_settings" onClick="togglePayOSSetting()" style="border: none; text-decoration: underline; color: #2271b1">' . __('Enter information of payOS', 'payos') .'</button>
			</td>
		</tr>
		<tbody id="payos_gateway_settings_group" style="display:none">
			<tr valign="top">
				...
				<td class="forminp" id="payos_gateway_settings">
					...
					<input id="show_client_id" onclick="showClientId()" type="button" style="border: none; background-color: #f0f0f1; min-width: 45px !important; color: #056fca; border-radius: 5px; padding: 6px;" value="' . __('Show', 'payos') . '" /> 
				</td>
			</tr>
			<tr valign="top">
				...
				<td class="forminp" id="payos_gateway_settings">
					...
					<input id="show_api_key" onclick="showApiKey()" type="button" style="border: none; background-color: #f0f0f1; min-width: 45px !important; color: #056fca; border-radius: 5px; padding: 6px;" value="' . __('Show', 'payos') . '" /> 
				</td>
			</tr>
			<tr valign="top">
				...
				<td class="forminp" id="payos_gateway_settings">
					...
					<input id="show_checksum_key" onclick="showChecksumKey()" type="button" style="border: none; background-color: #f0f0f1; min-width: 45px !important; color: #056fca; border-radius: 5px; padding: 6px;" value="' . __('Show', 'payos') . '" /> 
					<br>
					<div style="margin-top: 8px">
						<input type="submit" name="submit" id="submit" class="button button-primary" value="' . __('Check connection payOS', 'payos') .'" style="border: none; background-color: #056fca; color: white; border-radius: 5px; min-width: 95px !important;">
						<a href="https://a6e3495e.payos-docs.pages.dev/cong-cu/cong-thanh-toan/huong-dan-lay-client-id-va-api-key" onclick="window.open(this.href); return false;" onkeypress="window.open(this.href); return false;" style="margin-left: 5px; border: none; color: #2271b1; text-decoration: underline; line-height: 30px">' . __('Instructions to get Client ID and API Key', 'payos') . '</a>
					</div>
					';		
				$payment_gateway_config .= '
				</td>
		...
		<tr>
			...
			<td id="payos_gateway_settings">
				...
				<label for="transaction_prefix" style="font-size: 13px; font-style: oblique;">' . esc_html(__("Maximum 3 characters, no spaces and no special characters. If contained, it will be deleted. Please do not prefix starting with FT, TF, TT, VQR", "payos")) . '</label>
			</td>
		</tr>
		<tr>
		...
		<script type="text/javascript">
			function showDetailGateway() {
				...
				if (dots.style.display === "none") {
					...
					button.innerHTML = "' . __('Show less', 'payos') . '";
				} else {
					dots.style.display = "none";
					button.innerHTML = "';
				if ($this->payos_gateway_settings['link_webhook'] == 'no') 
					$payment_gateway_config .= __('No Connection', 'payos');
				else 
				 	$payment_gateway_config .= __('Đã kết nối', 'payos');  
				$payment_gateway_config .=	'";
				}
			}
			...
			function showClientId() {
				...
				if (clientId.type === "password") {
					clientId.type = "text";
					showClientIdBtn.value = "' . __('Hide', 'payos') . '";
				} else {
					clientId.type = "password";
					showClientIdBtn.value = "' . __('Show', 'payos') . '";
				}
			}
			...
			function showApiKey() {
				...
				if (apiKey.type === "password") {
					apiKey.type = "text";
					showApiKeyBtn.value = "' . __('Hide', 'payos') . '";
				} else {
					apiKey.type = "password";
					showApiKeyBtn.value = "' . __('Show', 'payos') . '";
				}
			}
			...
			function showChecksumKey() {
				...
				if (checksumKey.type === "password") {
					...
					showChecksumKeyBtn.value = "' . __('Hide', 'payos') . '";
				} else {
					checksumKey.type = "password";
					showChecksumKeyBtn.value = "' . __('Show', 'payos') . '";
				}
			}
			...
			function togglePayOSSetting() {
				...
				var vietQRGatewayGroup = document.getElementById("payos_gateway_settings_group");
				if (vietQRGatewayGroup.style.display == "none") {
					vietQRGatewayGroup.style.display = "table-row-group";
				}
				else {
					vietQRGatewayGroup.style.display = "none";
				}
			}
		</script>
		';
...
<?php
	...
	public function get_payos_payment_url($order_id)
	{
		...
		if ($order->get_payment_method() === 'payos') {
			if (isset($payos_gateway_settings['use_payment_gateway']) && $payos_gateway_settings['use_payment_gateway'] == 'yes') {
				...
				$redirect_url = wc_get_checkout_url() . 'order-received/' . $order->get_id() . '/?' . $_SERVER['QUERY_STRING'];
	...
	public function use_payment_gateway_template($status) {
		...
		switch($status) {
			case 'PAID':
				...
				$msg_txt = __('Order has been successfully paid.', 'payos');
				break;
			case 'CANCELLED':
				$img_file_name = 'failed.png';
				$msg_txt = __('Payment canceled', 'payos');
				break;
			default:
				$img_file_name = 'info.png';
				$msg_txt = __('Your order will be automatically confirmed once the payment transaction is received.', 'payos');
				break;
		}
	...
	public function casso_get_order_statuses_after_paid()
	{
		...
		$statuses['wc-default'] = __('Default', 'payos');
	...
	public function verify_payment_handler() 
	{
		...
		if ($body['desc'] == 'Giao dich thu nghiem' || $body['data']['reference'] == 'MA_GIAO_DICH_THU_NGHIEM') {
			echo "True";
			die();
		}
		...
		$transaction_note = sprintf(__('<b>Transaction Infomation:</b> <br> Order Code: %s <br> Amount: %s <br> Description: %s <br> Account Number: %s <br> Reference: %s', 'payos'), 
